...
export default function ProductsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetchWithAuth('/api/admin/customer-prices', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleProductSave = async (data: ProductFormValues) => {
    try {
      if (sheetMode === 'create') {
        ...
        const response = await fetchWithAuth(`/api/products/${currentProduct.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data),
        });
  ...
  const handleBulkDeleteConfirm = async () => {
    setIsDeleting(true);
    try {
      const response = await fetchWithAuth('/api/products/bulk-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
