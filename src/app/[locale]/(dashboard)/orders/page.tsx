...
export default function OrdersPage() {
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const response = await fetchWithAuth('/api/orders/bulk-delete', {
        method: 'POST',
  ...
  const handleBulkInvoice = async (separate: boolean = false) => {
    try {
      ...
      const response = await fetchWithAuth('/api/orders/bulk-schet-faktura', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleSaveOrder = async (data: any) => {
    try {
      if (editingOrder?.id) {
        ...
        const res = await fetchWithAuth(`/api/orders/${editingOrder.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
