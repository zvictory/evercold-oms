...
export function DeliveryCard({ delivery, variant = 'compact', onStartDelivery }: DeliveryCardProps) {
  ...
  return (
    <div className="border border-slate-200 rounded-lg p-4 bg-white hover:shadow-md transition">
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-sm text-slate-600">{(t as any)('Driver.route.orderNumber', { number: order.orderNumber })}</p>
            <span className="text-xs text-slate-500 font-medium">{(t as any)('Driver.route.stopNumberShort', { number: delivery.stops.indexOf(firstStop) + 1 })}</span>
            <span className={`px-2 py-1 text-xs font-semibold rounded border ${statusBadge.className}`}>
              {statusBadge.text}
            </span>
          </div>
        </div>
      ...
      {firstBranch?.deliveryAddress && (
        <div className="mb-3 bg-slate-50 rounded-lg p-3">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <p className="text-xs text-slate-600 mb-1">📍 {(t as any)('Driver.delivery.deliveryAddress')}</p>
              <p className="text-sm font-medium text-slate-900">{firstBranch.deliveryAddress}</p>
              <span className="text-xs text-slate-500 font-medium">{(t as any)('Driver.delivery.items', { count: totalItems })}</span>
            </div>
            {(firstBranch.latitude || firstBranch.deliveryAddress) && (
              <button
                onClick={handleNavigate}
                className="px-3 py-1.5 bg-sky-600 text-white text-xs font-semibold rounded hover:bg-sky-700 transition whitespace-nowrap"
              >
                {(t as any)('Driver.delivery.navigate')}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="mb-3 flex items-center justify-between gap-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-900">
              {firstBranch.contactPerson || (t as any)('Driver.delivery.contact')}
            </span>
          </div>
          {firstBranch.phone && (
            <button
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs text-slate-600">
            📦 {(t as any)('Driver.delivery.products')} ({(t as any)('Driver.delivery.productsCount_' + (orderItems.length === 1 ? 'one' : 'other'), { count: orderItems.length })})
          </p>
          {hasMoreProducts && (
            <button
            ...
            >
              {showAllProducts ? (t as any)('Driver.delivery.hide') + ' ▲' : (t as any)('Driver.delivery.showAll') + ' ▼'}
            </button>
          )}
        </div>
        <div className="space-y-1">
          ...
          {!showAllProducts && hasMoreProducts && (
            <p className="text-xs text-slate-500 italic">
              {(t as any)('Driver.delivery.moreProducts', { count: remainingCount })}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button
          onClick={handleStart}
          className="flex-1 py-2.5 bg-sky-600 text-white text-sm font-bold rounded-lg hover:bg-sky-700 transition"
        >
