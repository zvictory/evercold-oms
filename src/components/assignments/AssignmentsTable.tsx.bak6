...
export function AssignmentsTable({
  ...
  const handleAssignVehicle = useCallback(
    async (vehicleId: string) => {
      ...
      try {
        ...
        toast.success(t('Assignments.messages.assignSuccess'))
        await onDataRefresh()
  ...
  const handleUnassignVehicle = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t('Assignments.messages.unassignSuccess'))
      await onDataRefresh()
  ...
  const handleBulkUnassign = useCallback(async () => {
    ...
    try {
      const promises = Array.from(selectedDrivers).map(driverId =>
        fetchWithAuth(`/api/assignments/${driverId}`, { method: 'DELETE' })
      )
      ...
      if (failed > 0) {
        toast.error(t('Assignments.messages.bulkUnassignPartialError', { count: String(failed) }))
      } else {
        toast.success(t('Assignments.messages.bulkUnassignSuccess', { count: String(selectedDrivers.size) }))
      }
    ...
    <>
      ...
      <div className="mb-6 grid grid-cols-4 gap-4">
        <div className="rounded-lg bg-white p-4 border border-slate-200">
          <p className="text-sm text-slate-600">{t('Assignments.stats.totalDrivers')}</p>
          <p className="text-2xl font-bold tabular-nums text-slate-900">{drivers.length}</p>
        </div>
        <div className="rounded-lg bg-white p-4 border border-slate-200">
          <p className="text-sm text-slate-600">{t('Assignments.stats.totalVehicles')}</p>
          <p className="text-2xl font-bold tabular-nums text-slate-900">{vehicles.length}</p>
        </div>
        <div className="rounded-lg bg-white p-4 border border-slate-200">
          <p className="text-sm text-slate-600">{t('Assignments.stats.assigned')}</p>
          <p className="text-2xl font-bold tabular-nums text-emerald-600">{assignedVehicles.length}</p>
        </div>
        <div className="rounded-lg bg-white p-4 border border-slate-200">
          <p className="text-sm text-slate-600">{t('Assignments.stats.unassigned')}</p>
          <p className="text-2xl font-bold tabular-nums text-amber-600">{unassignedVehicles.length}</p>
        </div>
      </div>
      ...
      {selectedDrivers.size > 0 && (
        <div className="mb-4 rounded-lg bg-sky-50 border border-sky-200 p-4 flex items-center justify-between">
          <p className="text-sm text-slate-700">
            {t('Assignments.bulk.driversSelected', { count: String(selectedDrivers.size) })}
          </p>
          <Button
          ...
          >
            ...
            {t('Assignments.bulk.unassignSelected')}
          </Button>
        </div>
      )}
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/50 border-slate-100">
              ...
              <TableHead className="font-semibold text-slate-900">{t('Assignments.table.driverName')}</TableHead>
              <TableHead className="font-semibold text-slate-900">{t('Assignments.table.phone')}</TableHead>
              <TableHead className="font-semibold text-slate-900">{t('Assignments.table.license')}</TableHead>
              <TableHead className="font-semibold text-slate-900">{t('Assignments.table.currentVehicle')}</TableHead>
              <TableHead className="font-semibold text-slate-900">{t('Assignments.table.model')}</TableHead>
              <TableHead className="text-right font-semibold text-slate-900">{t('Assignments.table.actions')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {drivers.map(driver => (
                ...
                <TableCell>
                  {driver.plateNumber ? (
                    ...
                    <Badge variant="outline" className="text-slate-500">
                      {t('Assignments.table.unassignedBadge')}
                    </Badge>
                  )}
                </TableCell>
