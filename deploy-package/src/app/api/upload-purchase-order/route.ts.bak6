    ...
    for (const parsedOrder of parsedOrders) {
      try {
        ...
        let customer = await prisma.customer.findFirst({
          where: {
            OR: [
              { name: { contains: parsedOrder.customerName, mode: 'insensitive' } },
            ...
            if (item.branchCode) {
              ...
              if (branchCode) {
                branch = await prisma.customerBranch.findFirst({
                  where: {
                    OR: [
                      { branchName: { contains: branchCode, mode: 'insensitive' } },
                      { branchCode: { contains: branchCode, mode: 'insensitive' } },
                      { oldBranchCode: { contains: branchCode, mode: 'insensitive' } },
                    ],
                  },
                });
