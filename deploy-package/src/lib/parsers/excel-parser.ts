...
function parseDetailedOrder(rows: any[], receivedDate: Date): ParsedOrder {
  ...
  const terminatorPatterns = ['Итого', 'Всего', 'Total', ''];

  for (let i = 6; i < rows.length - 1; i++) {
    const rowData = extractRowData(rows[i]);
    const productName = (rowData[1] || '').trim();

    if (productName && !terminatorPatterns.includes(productName)) {
...
function parseRegistry(rows: any[], receivedDate: Date): ParsedRegistry {
  ...
  const registryTerminatorPatterns = ['Итого', 'Всего', 'Total', ''];
  const productRows: any[] = [];
  for (let i = 3; i < rows.length; i++) {
    const rowData = extractRowData(rows[i]);
    const productName = (rowData[1] || '').trim();
    if (productName && !registryTerminatorPatterns.includes(productName)) {
