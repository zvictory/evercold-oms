...
export function handleDriverAuthError(error: unknown) {
...
/**
 * Extract and validate driver session from request.
 * Checks both Authorization header (Bearer token) and cookies (driverToken).
 * Throws DriverAuthError if unauthorized.
 */
export async function requireDriver(request: NextRequest) {
  const token =
    request.headers.get('authorization')?.replace('Bearer ', '') ||
    request.cookies.get('driverToken')?.value;

  if (!token) {
    throw new DriverAuthError('Unauthorized: No token provided', 401);
  }

  const session = await validateToken(token);
  if (!session) {
    throw new DriverAuthError('Unauthorized: Invalid or expired session', 401);
  }

  if (session.driver.status !== 'ACTIVE') {
    throw new DriverAuthError('Forbidden: Driver account is not active', 403);
  }
